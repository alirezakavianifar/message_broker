Below is a clear, step-by-step deliverable plan (phases, tasks, artifacts, and acceptance criteria) based on the original specification and the employer’s answers. It assumes: FastAPI backend, Redis queue (persistent), MySQL, Mutual TLS for client → proxy → main-server traffic, Python client script, Linux (Debian/Ubuntu) servers, no Docker, manual testing only, web portal (user + admin), and certificate issuance on the main server via OpenSSL.
...
1. **Task:** Create project repository layout and onboarding docs.
   ...
   **Acceptance:** Repo exists, instructions to set up a local dev environment on Debian workstations are validated.
...
2. **Artifacts:** `generate_cert.sh`, `revoke_cert.sh`, README for issuing and installing certs.
...
3. **Artifacts:** FastAPI app, Swagger UI endpoint, config files (TLS cert paths, Redis/MYSQL connection config), systemd service file for Debian.
...
3. **Artifacts:** Worker code, sample systemd service file for worker, configuration sample, concurrency instructions.
...
3. **Artifacts:** Portal source, build steps, nginx/site conf for Debian, credentials bootstrap doc.
...
1. **Task:** Prepare deployment scripts for Debian servers (no Docker):

   * systemd unit files for proxy, workers, main server, and portal; Nginx reverse proxy config for portal; TLS certificate placement instructions.
...
2. **Task:** Deploy to a test Debian server (provided domain name), configure domains as requested (parameterize domain so same code works for multiple domains).
3. **Task:** Provide backup and restore instructions for MySQL and Redis.
4. **Artifacts:** Deployment guide (step-by-step), systemd files, Nginx config snippets, production config examples.
...
* **No Docker & no automated tests:** Implementation uses systemd services on Debian; testing is manual per employer request.
...
* Certificate generation and revocation scripts (`openssl` based).
* Client Python example script that posts messages (two variables).
* Deployment guide for Debian (systemd service files, Nginx).
